module Q3DB
  ...
  class QuakeEntry
    ...
    def create_kill( elapsed, killer_id, killed_id, weapon_id, killer_name, killed_name, weapon_name )
      ...
      Stat.where(:nickname => score.killer.nickname).first_or_create
      .increment! :winnings unless bots.push(world).include? score.killer.nickname

      Stat.where(:nickname => score.killed.nickname).first_or_create
      .increment! :defeats  unless bots.include?             score.killed.nickname
    end
